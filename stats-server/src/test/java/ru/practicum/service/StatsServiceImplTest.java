...
public class StatsServiceImplTest {
    ...
    void saveHitTest() {
        ...
        assertEquals(service.saveHit(hitDto).getApp(), hitDto.getApp());
    }
    ...
    void getStatsTest() {
        ...
        assertEquals(statsDto, service.getStats(start, end, null, false).get(0));

        when(repository.findUriStats(any(), any(), any()))
                .thenReturn(List.of(statsDto));

        assertEquals(statsDto, service.getStats(start, end, List.of(hitDto.getUri()), false).get(0));

        when(repository.findUniqueIpStats(any(), any()))
                .thenReturn(List.of(statsDto));

        assertEquals(statsDto, service.getStats(start, end, null, true).get(0));

        when(repository.findUniqueIpAndUriStats(any(), any(), any()))
                .thenReturn(List.of(statsDto));

        assertEquals(statsDto, service.getStats(start, end, List.of(hitDto.getUri()), true).get(0));
    }
